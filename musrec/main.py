...
def main():
    ...
    recParser.add_argument(
        "--format", "-f", choices = ["wav", "mp3", "flac", "ogg"], default=load()["outputFormat"], help=f"Output file format; (default: .{load()["outputFormat"]}) (supported: .wav (automatic metadata not supported), .mp3, .flac, .ogg (no album art))."
    )
    recParser.add_argument(
